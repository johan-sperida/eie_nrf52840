...
#include "sceneObjects/lossState.h"
...
// Typedefs
enum game_states { MENU_STATE, TRANSITION_STATE, MINIGAME_STATE, LOSS_STATE };

// Local vars
// Define the state table using the enum values and naming conventions
static const struct smf_state game_states[] = {
    [MENU_STATE] = SMF_CREATE_STATE(menu_state_entry, menu_state_run,
                                    menu_state_exit, NULL, NULL),
    [TRANSITION_STATE] =
        SMF_CREATE_STATE(transition_state_entry, transition_state_run,
                         transition_state_exit, NULL, NULL),
    [MINIGAME_STATE] =
        SMF_CREATE_STATE(minigame_state_entry, minigame_state_run,
                         minigame_state_exit, NULL, NULL),
    [LOSS_STATE] = SMF_CREATE_STATE(loss_state_entry, loss_state_run,
                                    loss_state_exit, NULL, NULL),
};
...
static scene transitionStateScene = {.enterFunc = transition_enter,
                                     .runFunc = transition_run};

static scene lossStateScene = {.enterFunc = &loss_enter};

// minigames ---------------------------------
static scene countMinigameScene = {.enterFunc = &count_enter,
                                   .runFunc = &count_run};

static minigameStateScene countMinigame = {.minigame = &countMinigameScene,
                                           .name = "Count!",
                                           .controls = "[Use Screen]"};

static scene spamMinigameScene = {.enterFunc = &spam_enter,
                                  .runFunc = &spam_run};

static minigameStateScene spamMinigame = {.minigame = &spamMinigameScene,
                                          .name = "Spam!",
                                          .controls = "[Use Screen]"};

static scene alignMinigameScene = {.enterFunc = &align_enter,
                                   .runFunc = &align_run};

static minigameStateScene alignMinigame = {.minigame = &alignMinigameScene,
                                           .name = "Align!",
                                           .controls = "[Use Sensor]"};

// The pool from which to pick a minigame
static minigameStateScene *minigamePool[MINIGAMES_AMOUNT] = {&countMinigame,
                                                             &spamMinigame};
//---------------------------------

static scene menuStateScene = {.enterFunc = &menu_enter, .runFunc = &menu_run};
...
static void transition_state_entry(void *o) {
  ...
  k_msleep(1000 * SLEEP_MS * time_multiplier);
}
...
static enum smf_state_result minigame_state_run(void *o) {
  ...
  time_multiplier = (float)8.0 / (float)(game_state_object.levelCount + 10.0);
  int ret = game_state_object.activeScene->runFunc(
      game_state_object.activeScene, NULL);
  if (!ret) {
    game_state_object.levelCount += 1;
    scene_Load(&greenTransition, true);
    smf_set_state(SMF_CTX(&game_state_object), &game_states[TRANSITION_STATE]);
  } else {
    scene_Load(&redTransition, true);
    if (game_state_object.lives == 1) {
      printk("loss\n");
      ...
      smf_set_state(SMF_CTX(&game_state_object),
...
static void loss_state_entry(void *o) {
  printk("enter loss <------------------\n");
  scene_Init(&lossStateScene);
  lossStateScene.enterFunc(&lossStateScene, &game_state_object);
}
