...
int menu_run(void *v_scene, void *o) {
  ...
  lv_obj_set_style_bg_color(p_scene->parent, lv_color_hex(0xFFFFFF),
                            LV_PART_MAIN); // WHITE

  lv_obj_t *menu_btn = lv_button_create(p_scene->parent);
  lv_obj_align(menu_btn, LV_ALIGN_CENTER, 0, 0);
  lv_obj_set_size(menu_btn, LV_PCT(50), LV_PCT(30));
  lv_obj_t *button_label = lv_label_create(menu_btn);
  lv_label_set_text(button_label, "Start game");
  lv_obj_align(button_label, LV_ALIGN_CENTER, 0, 0);

  int initial_state = 0;
  lv_obj_t *data_obj = lv_data_obj_create_alloc_assign(menu_btn, &initial_state,
                                                       sizeof(initial_state));
  lv_obj_add_event_cb(menu_btn, menu_button_callback, LV_EVENT_CLICKED,
                      data_obj);
